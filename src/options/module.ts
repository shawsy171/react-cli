...
export default class CreateModule {
  ...
  constructor(
    ...
    this.dir = path.join(LOCATION + "/src/modules", moduleName);
  }
  ...
  createFolder() {
    ...
    createFolder(LOCATION + "/src/", this.PAGES);
  ...
  createPageComponent() {
    ...
    createFile(
      pageLocation,
      pageTemplate(this.pageName, this.moduleName, this.componentName),
    );
  }
  ...
  createComponent() {
    ...
    createFile(componentLocation, componentTemplate(this.componentName));
    createFile(componentTestLocation, testTemplate(this.componentName));
  }
  ...
  addRouteToRoutes() {
    ...
    fs.readFile(this.projectRoutesPath, "utf-8", (err, fileData) => {
      ...
      const modifiedFileData =
        fileData.slice(0, searchLineIndex - 1) +
        newLine +
        "\n" +
  ...
  addRouteToRouteObject() {
    ...
    try {
      ...
      const searchObjectEndIndex =
        fileData.indexOf("};", searchObjectIndex) + 2;
      ...
      const modifiedFileData =
        fileData.slice(0, searchObjectEndIndex - 2) + newLine + "\n" + fileEnd;
